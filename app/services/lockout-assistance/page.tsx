...
export const metadata: Metadata = {
  title: "Car Lockout Assistance Stockport | 25-Min Response, No Call-Out Fee",
  description:
    "Locked out of your car in Stockport? Vikki Heaton arrives in 25 minutes, 24/7. Non-destructive entry. No call-out fee. Fully insured. Call 07309903243 now.",
  robots: {
    index: true,
    follow: true,
  },
  openGraph: {
    title: "Car Lockout Assistance Stockport | 25-Min Response, No Call-Out Fee",
    description: "Locked out of your car? 25-min response across Stockport. Non-destructive entry. No call-out fee. 24/7. Call 07309903243.",
...
const lockoutFaqData = [
  {
    question: "How quickly can a locksmith reach me in Stockport for a car lockout?",
    answer: "Car Keys in Stockport averages a 25-minute response time for vehicle lockouts across Stockport and Greater Manchester. Vikki Heaton operates a fully equipped mobile unit and travels directly to your location — home, work, car park, or roadside — 24 hours a day, 7 days a week.",
  },
  {
    question: "Will a locksmith damage my car when opening it?",
    answer: "No. Car Keys in Stockport uses non-destructive entry techniques — specialist tools designed to open vehicle doors without any damage to the lock mechanism, door frame, or glass. This is why calling a professional locksmith is always preferable to attempting to force a window or door yourself.",
  },
  {
    question: "How much does car lockout assistance cost in Stockport?",
    answer: "Car lockout assistance from Car Keys in Stockport costs £60–£150 depending on the vehicle and method required. There is no call-out fee. You receive a fixed price before any work begins. This is typically 40–60% cheaper than dealer lockout services and far cheaper than a broken window.",
  },
  {
    question: "Can you help if my keys are locked inside the car?",
    answer: "Yes, this is one of the most common emergency calls. Car Keys in Stockport can open any vehicle with keys locked inside using non-destructive techniques. Once inside, if needed, a replacement key can also be cut and programmed on-site.",
  },
];

const lockoutServiceSchema = {
  "@context": "https://schema.org",
  "@type": "Service",
  "@id": "https://carkeysinstockport.co.uk/services/lockout-assistance#service",
  name: "Car Lockout Assistance Stockport",
  description: "Professional 24/7 car lockout assistance across Stockport and Greater Manchester. Non-destructive entry, 25-minute average response time. No call-out fee. Operated by Vikki Heaton, fully insured auto locksmith.",
  provider: {
    "@type": "LocalBusiness",
  ...
  serviceType: "Vehicle Lockout Assistance",
  areaServed: [
...
export default function LockoutAssistancePage() {
  return (
    <>
      <Script
        id="lockout-service-schema"
        type="application/ld+json"
        dangerouslySetInnerHTML={{ __html: JSON.stringify(lockoutServiceSchema) }}
      />
      <Script
        id="lockout-faq-schema"
        type="application/ld+json"
        dangerouslySetInnerHTML={{ __html: JSON.stringify(faqSchema(lockoutFaqData)) }}
      />
    <main className="bg-slate-900 text-white pt-20">
      ...
      <section className="section-padding bg-slate-900">
        <div className="container-custom max-w-3xl mx-auto">
          <h2 className="text-2xl font-bold text-center mb-8">Car Lockout FAQs — Stockport</h2>
          <div className="space-y-6">
            {lockoutFaqData.map((faq, i) => (
              <article key={i} className="card p-6" itemScope itemType="https://schema.org/Question">
                <h3 className="text-lg font-bold text-white mb-3" itemProp="name">{faq.question}</h3>
                <div itemScope itemType="https://schema.org/Answer" itemProp="acceptedAnswer">
                  <p className="text-gray-300 text-sm leading-relaxed" itemProp="text">{faq.answer}</p>
                </div>
              </article>
            ))}
          </div>
